...
let maxReplacementLength: number | undefined;
let replacementMap: Map<string, string> | undefined;

/**
 * Convert a string of kana to romaji.
 *
 * @param kana The kana to convert.
 *
 * @returns The romaji equivalent of the input kana.
 */
export function toRomaji(kana: string): string {
  if (!maxReplacementLength) {
    maxReplacementLength = Math.max(...replacements.map(([a]) => a.length));
  }
  if (!replacementMap) {
    replacementMap = new Map(replacements);
  }
  ...
  while (i < hiragana.length) {
    ...
    if (firstCharCode < 0x3041) {
      explode();
      result += hiragana.substring(i, i + 1);
      i++;
      continue;
    }
    ...
    while (substringLength) {
      const substring = hiragana.substring(i, i + substringLength);
